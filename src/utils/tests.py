...
train_dir      = "data/processed/dev"
train_protocol = "data/raw/ASVspoof2019_LA_cm_protocols/ASVspoof2019.LA.cm.dev.trl.txt"

ds = ASVSpoofDataset(train_dir, train_protocol, mode="train")
labels = [label for _, label in ds.items]

unique, counts = np.unique(labels, return_counts=True)
counts_dict = dict(zip(unique, counts))
bona_fide = counts_dict.get(1, 0)
spoof     = counts_dict.get(0, 0)
ratio = bona_fide / spoof if spoof > 0 else float("inf")

print(f"bona-fide: {bona_fide}, spoof: {spoof}")
print(f"Соотношение bona-fide / spoof = {ratio:.3f} ({bona_fide}:{spoof})")
